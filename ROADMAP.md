...
- **Frontend**: rebuilt from scratch in `frontend/` (React 19 + TypeScript + Vite + Tailwind v4 + shadcn/ui). Phase B (foundation: auth pages, app shell, routing) and Phase C (all core feature screens: Products, Warehouses, Stock Movements, Sales, Team, Reports, Businesses, Platform Admin) are both done and verified end-to-end in a real browser against the real backend. The auth pages were also redesigned mid-project into a glassmorphic style with a light/dark theme toggle, per Victor's request — see the Phase C writeup.
...
- [ ] Phase D — Polish & Reliability: loading/error states consistency pass, receipt print flow, responsive layout for tablet/phone (shop-floor use), basic accessibility pass.
...
- [ ] Consider adding a committed automated frontend test suite (Playwright or Vitest+RTL) — Phase C verification so far has been thorough but ad-hoc, not regression-proof.
