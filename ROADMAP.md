...
- **Frontend**: rebuilt from scratch in `frontend/` (React 19 + TypeScript + Vite + Tailwind v4 + shadcn/ui). Phase B (foundation), Phase C (all core feature screens), the auth glassmorphic redesign + theme toggle, an app-wide frontend rework, Phase D (polish & reliability), and a first page-by-page interaction pass on Register are all done and verified end-to-end in a real browser against the real backend. **Current IA**: login → `/` is a business-picker landing page (card grid, every role, every login) → selecting a business lands on `/pos`, a cash-register-style Register screen that is now the business "home" (replaced the old Sales/Transactions list+dialog). The Register itself is search-driven (type a product, see live stock, enter quantity, add), supports per-line discounts (shown only when explicitly applied, never on a manually-edited price), Cash/Transfer/Credit payment with a mandatory customer name for Credit sales, and a post-sale "Print receipt / New sale" popup. Dashboard, Products, Warehouses, Stock Movements, Team, Reports, and Businesses management (folded into the picker page) are all still reachable via the sidebar (or a mobile hamburger drawer below `lg`), just no longer the default landing. Visual language is glassmorphic (translucent blurred panels, near-black cinematic dark mode) consistently across the whole app now — picker/POS/Dashboard got a full dedicated redesign, and the remaining screens (Products, Warehouses, Stock Movements, Team, Reports, Platform Admin) were brought in line via shared-primitive updates (glass `Dialog`/`AlertDialog`, `PageHeader` typography, gradient `Tabs` active state). App now has a global error boundary, consistent query-error states with retry, real receipt printing, and responsive/mobile layout.
...
- [ ] Continue the page-by-page UX pass Victor is doing post-visual-rework — Register is done; Products, Warehouses, Stock Movements, Team, Reports, and Platform Admin haven't had this deeper interaction-level look yet (only the shared-primitive visual pass).
- [ ] Consider a real credit/debtor ledger (amounts owed, repayment/settlement tracking) if Victor asks for it once he's used the Credit payment method in practice.
