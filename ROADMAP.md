...
- **Frontend**: rebuilt from scratch in `frontend/` (React 19 + TypeScript + Vite + Tailwind v4 + shadcn/ui). Phase B (foundation), Phase C (all core feature screens), the auth glassmorphic redesign + theme toggle, an app-wide frontend rework, and Phase D (polish & reliability) are all done and verified end-to-end in a real browser against the real backend. **Current IA**: login → `/` is a business-picker landing page (card grid, every role, every login) → selecting a business lands on `/pos`, a cash-register-style Register screen that is now the business "home" (replaced the old Sales/Transactions list+dialog). Dashboard, Products, Warehouses, Stock Movements, Team, Reports, and Businesses management (folded into the picker page) are all still reachable via the sidebar (or a mobile hamburger drawer below `lg`), just no longer the default landing. Visual language is glassmorphic (translucent blurred panels, near-black cinematic dark mode) across the whole shell; only the picker/POS/Dashboard screens got a full dedicated redesign pass so far, everything else inherited a partial facelift from the shared `Card` primitive change. App now has a global error boundary, consistent query-error states with retry, real receipt printing, and responsive/mobile layout.
...
- [ ] Extend the glass redesign to Products, Warehouses, Stock Movements, Team, Reports, and Platform Admin (deferred from both the frontend rework and Phase D).
...
- [ ] Phase E — Deployment & Ops (per the Phase 3 master plan) once the above polish/visual items are far enough along to be worth shipping somewhere real.
