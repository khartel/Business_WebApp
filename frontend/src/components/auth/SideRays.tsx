...
import "@/components/auth/SideRays.css"
...
/**
 * WebGL "light beam" backdrop (via ogl), adapted from React Bits' SideRays.
 * Pauses rendering (via IntersectionObserver) when scrolled out of view, and
 * never initializes at all under `prefers-reduced-motion: reduce` - it's
 * decorative only, and callers should treat it the same as the CSS
 * `motion-safe:` aurora blobs it's meant to sit alongside.
 */
    ...
    const initializeWebGL = async () => {
      ...
      cleanupFunctionRef.current = () => {
        ...
        try {
          ...
          // context may already be gone (e.g. tab discarded it) - nothing to clean up
        }
